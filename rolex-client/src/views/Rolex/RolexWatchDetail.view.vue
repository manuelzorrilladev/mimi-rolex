...
watch(isReady, () => {
  console.log(family.value.idName);
  useHead(
    {
      script: [
        `var digitalDataLayer = ${JSON.stringify(digitalDataLayer(family.value.idName, rmc.value))}; `,
      ],
    },
    { head },
  );
      ...
      <template #content>
        <div v-if="isReady" class="bg-rolex-brown-light-2">
          ...
          <section class="relative md:h-[85vh]">
            ...
            >
              <img
                :src="`${storageRoute}/${state.getAll.modelo}-showcase.webp`"
              ...
              >
                <slide v-for="slide in 4" :key="slide">
                  <div class="flex justify-center w-full">
                    <img
                      :src="sliderItems(slide - 1, state.getAll.modelo)"
                      :alt="`slide-${slide}`"
                      class="w-10/12"
                      :class="slide - 1 == 0 ? 'w-2/3' : 'w-full'"
                    />
                  </div>
                </slide>
              </carousel>
            ...
            <div
              class="md:absolute w-full h-full top-0 flex items-center justify-center pl-10 md:pl-0 pb-10 md:pb-0"
            >
              <div class="font-helvetica w-full md:w-[85%]">
                <div class="md:w-1/3">
                  ...
                  <h1 class="font-thin text-rolex-brown text-sm">
                    {{ state.getAll.cajaDelModelo }}
                  </h1>
                  <h1 class="font-thin text-rolex-brown text-sm">
                    {{ state.getAll.modelo.toUpperCase() }}
                  </h1>
                  ...
                  <Transition name="fade">
                    ...
                    >
                      <span v-if="checkLocation">
                        Descargue la lista de precios recomendados de su país
                        para encontrar la información de los precios específicos
                        de su país.
                      </span>
                      <span v-else>
                  ...
                  >
                    <div
                      class="flex flex-nowrap md:grid md:grid-cols-2 md:grid-rows-2 gap-4 md:gap-y-2 pb-4 md:pb-0"
                    >
                      <button
                        @click="CTATrack('call')"
          ...
          <section v-if="isDesktop" class="flex w-full justify-center py-14">
            <div class="md:w-[85%] flex justify-center gap-1 pr-3">
              <img
                v-for="item in 3"
                :src="`${storageRoute}${state.getAll.modelo}-slider-${item}.webp`"
                alt=""
                ...
                <div class="w-10/12 flex flex-col items-center justify-center">
                  <header class="border-t py-4 w-full">
                    ...
                    <Transition name="appear">
                      <p
                        v-if="isVisible(0)"
                        class="font-thin h-[170px] text-rolex-brown text-sm overflow-hidden duration-300"
                      >
                        Para garantizar la precisión y la fiabilidad de sus
                        relojes, Rolex somete a cada reloj, tras el ensam-blaje,
                        a una serie de pruebas estrictas. Todos los nuevos
                        relojes adquiridos en uno de los Distribui-dores
                        Oficiales de la marca incluyen una garantía
                        internacional de cinco años. Cuando compra un Rolex, el
                        Distribuidor Oficial rellena y deja registrada la fecha
                        en la tarjeta de garantía Rolex, que certifica la
                        autenticidad de su reloj.
                      </p>
                    </Transition>
                  </header>
                  <header class="border-t py-4 w-full">
                    ...
                    <Transition name="appear">
                      <p
                        ...
                        class="font-thin h-[170px] text-rolex-brown text-sm overflow-hidden duration-300"
                      >
                        La garantía de cinco años que se aplica a todos los
                        modelos Rolex viene acompañada de un sello verde, un
                        símbolo de su estatus de Cronómetro Superlativo. Esta
                        exclusiva certificación da fe de que el reloj ha
                        superado con éxito una serie de controles finales
                        específicos llevados a cabo por Rolex en sus propios
                        laboratorios y según sus propios criterios, como
                        complemento de la certifica-ción oficial COSC de su
                        movimiento.
                      </p>
                    </Transition>
                  </header>
                  <header class="border-t py-4 w-full">
                    ...
                    <Transition name="appear">
                      <p
                        ...
                        class="font-thin h-[170px] text-rolex-brown text-sm overflow-hidden duration-300"
                      >
                        Todos los relojes Rolex se entregan en un precioso
                        estuche verde que protege y guarda la joya que anida en
                        su interior. Además, el estuche simboliza igualmente el
                        acto de regalar. Por ello, si usted quiere obsequiar a
                        alguien con un Rolex, es importante que lo primero que
                        vea el destinatario esté a la altura de su contenido.
                      </p>
                    </Transition>
                  </header>
                </div>
          ...
          >
            <section
              class="w-[85%] flex flex-col items-center bg-rolex-brown-light-1"
            >
              <div
                class="grid grid-cols-2 md:grid-cols-3 md:grid-rows-4 gap-y-5 gap-x-5 w-11/12 p-10"
              >
                <header class="">
              ...
              <div class="w-[85%] pb-10">
                <button
                  @click="downloadBrochure(state.details[0].brochure)"
                ...
                >
                  ...
                  >
                    <path
                      ...
                      class="fill-white group-hover:fill-rolex-green"
          ...
          >
            <div class="flex flex-col items-center py-[10vh]">
              ...
              <section class="w-10/12 md:w-1/2">
                <img
                  :src="`${storageRoute}/${state.headers.imagen1}.webp`"
                  alt="heading-img-1"
                  class="hidden md:block"
                />
                <img
                  :src="`${storageRoute}/${state.headers.imagen1}-mobile.webp`"
            ...
            <div class="flex flex-col items-center py-[10vh]">
              ...
              <section class="w-10/12 md:w-1/2">
                <img
                  :src="`${storageRoute}/${state.headers.imagen2}.webp`"
                  alt="heading-img-2"
                  class="hidden md:block"
                />
                <img
                  :src="`${storageRoute}/${state.headers.imagen2}-mobile.webp`"
            ...
            <div class="flex flex-col items-center py-[10vh]">
              ...
              <section class="w-10/12 md:w-1/2">
                <img
                  :src="`${storageRoute}/${state.headers.imagen3}.webp`"
                  alt="heading-img-3"
                  class="hidden md:block"
                />
                <img
                  :src="`${storageRoute}/${state.headers.imagen3}-mobile.webp`"
            ...
            >
              <header
                class="flex flex-col gap-8 w-10/12 pt-10 pb-20 justify-around items-center text-rolex-brown h-fit bg-rolex-brown-light-1"
              >
                <img
                ...
                <p
                  class="text-rolex-brown w-10/12 md:w-1/2 text-sm font-helvetica font-light text-center"
                >
          ...
          >
            <div class="w-10/12">
              <SectionNavigationCard
                :img="`banners/new-banner-collections-${family.idName}`"
                :link="{ name: `rolex-coleccion-${family.idName}` }"
                class="w-full"
              >
                <template #sub
                  >Rolex {{ capitalizeFirstLetter(family.nombre) }}</template
                >
