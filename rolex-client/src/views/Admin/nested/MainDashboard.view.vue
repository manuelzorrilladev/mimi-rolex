...
function getAllWatches() {
  ...
  adminDataServices
    ...
    .catch((e) => {
      console.log(e);
      errorHandlerMessage.value.watchesError =
        "Hubo un error obteniendo los relojes";
    });
}
...
function filterWatches() {
  displayWatches.value = displayWatches.value.filter((e) =>
    e.serie.includes(searchParam.value)
  );
}
...
function getMessages() {
  adminDataServices
    ...
    .catch((e) => {
      console.log(e);
      errorHandlerMessage.value.messagesError =
        "Hubo un error obteniendo los mensajes";
    });
}
...
function getRolexMessages() {
  adminDataServices
    ...
    .catch((e) => {
      console.log(e);
      errorHandlerMessage.value.rolexMessageError =
        "Hubo un error obteniendo los mensajes del formulario de Rolex";
    });
}
...
<template>
  <section class="w-full">
    ...
    <section
      class="flex flex-col w-full justify-center items-center overflow-hidden"
    >
      <DashboardCard class="col-span-2 w-11/12 min-w-11/12 shadow-xl">
        ...
        <template #content>
          <div class="h-[600px] pb-4 overflow-scroll w-full min-w-full flex">
            ...
            <table v-else-if="displayWatches.length > 0" class="w-full text-xl">
              <thead>
                <tr>
                  ...
                  <th
                    @click="sortByParam('serie', 2)"
                    :class="
                      sortingIndicator == 2
                        ? 'font-black underline'
                        : 'font-bold '
                    "
                    class="h-10 border border-white w-[30%] hover:underline cursor-pointer"
                  >
                    Serie
                  </th>
                  <th
                    @click="sortByParam('coleccion', 3)"
                    :class="
                      sortingIndicator == 2
                        ? 'font-black underline'
                        : 'font-bold '
                  ...
                  <th
                    @click="sortByParam('disponible', 5)"
                    :class="
                      sortingIndicator == 4
                        ? 'font-black underline'
                        : 'font-bold '
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="border border-white flex justify-center">
                    <img :src="value.img" alt="" class="w-2/3" />
                  </td>
                  <td class="border border-white w-[10%]">
                    ${{ value.precio }}
                  </td>
                  <td class="border border-white w-[10%]">
                    {{ value.disponible == 1 ? "Disponible" : "No disponible" }}
                  </td>
                  <td class="border border-white w-[20%]">
                    <p
                      @click="toggleCollection(value.id, value.coleccion)"
                      class="underline cursor-pointer"
                    >
                      {{
                        value.collection == "Tudor"
                          ? "Cambiar a Novedades"
                          : "Cambiar a Tudor"
                      }}
                    </p>
                    <p
                      @click="toggleAvailable(value.id)"
                      class="underline cursor-pointer"
                    >
                      {{ value.disponible == 1 ? "Desactivar" : "Activar" }}
                    </p>
                  </td>
                </tr>
              </tbody>
            ...
            <div
              v-else
              class="overflow-hidden h-full space-y-4 w-full border flex items-center"
            >
      ...
      <div class="w-11/12 flex gap-4 py-6">
        <DashboardCard class="w-1/2 shadow-xl">
          <template #title> Mensajes generales </template>
          <template #content>
            <div
              v-if="errorHandlerMessage.messagesError != ''"
              class="overflow-scroll h-[450px] space-y-4 flex justify-center items-center"
            >
              <h2 class="text-3xl font-bold">
                {{ errorHandlerMessage.messagesError }}
              </h2>
            </div>
            <div
            ...
            >
              <div
                v-for="(value, index) in messages.messages"
                class="border border-main-green bg-white rounded-xl shadow-sm p-3"
                :key="value"
              >
                ...
                <div
                  @click="activateMessageToggle(index, 'messages')"
        ...
        <DashboardCard class="w-1/2 shadow-xl">
          <template #title> Mensajes Rolex </template>
          <template #content>
            <div
              v-if="errorHandlerMessage.rolexMessageError != ''"
              class="overflow-scroll h-[450px] space-y-4 flex justify-center items-center"
            >
              <h2 class="text-3xl font-bold">
                {{ errorHandlerMessage.rolexMessageError }}
              </h2>
            </div>
            <div
              v-else-if="messages.rolexMessages.length > 0"
              class="overflow-scroll h-[450px] w-full space-y-4"
            >
              <div
                v-for="(value, index) in messages.rolexMessages"
                class="border border-main-green bg-white rounded-xl shadow-sm p-3"
                :key="value"
              >
                <h2 class="font-bold">{{ value.name }}</h2>
                ...
                <div
                  @click="activateMessageToggle(index, 'rolexMessages')"
                  class="h-8 w-[600px] whitespace-nowrap overflow-hidden hover:underline cursor-pointer relative italic text-lg text-ellipsis"
                >
                  {{ value.message }}
                </div>
    ...
    <section class="fixed z-50 bottom-10  w-full flex justify-center">
      <DashboardCard class="w-11/12">
        <template #content>
          <div class="flex flex-col w-full gap-2">
            <h2 class="font-montserrat text-xl font-bold">Acciones rápidas</h2>
            <div class="w-full flex gap-4 justify-between">
            
              <RouterLink
                :to="{ name: 'agregar-reloj' }"
                class="p-2 border-2 border-rolex-green rounded-xl bg-rolex-green text-white duration-200 hover:bg-white hover:text-dark-green flex items-center gap-2 w-1/4"
              >
              ...
              <button
                @click="modal = true"
                class="cursor-pointer p-2 border-2 border-rolex-green rounded-xl bg-rolex-green text-white duration-200 hover:bg-white hover:text-dark-green flex items-center gap-2 w-1/4"
              >
                <font-awesome-icon
              ...
              >
                <font-awesome-icon
                  :class="
                    iconCharge == 'globe' ? 'animate-none' : 'animate-spin'
                  "
                  :icon="['fas', iconCharge]"
                  class="text-3xl"
                />
                ...
                <RouterLink
                :to="{ name: 'ver-metricas' }"
                class="p-2 border-2 border-rolex-green rounded-xl bg-rolex-green text-white duration-200 hover:bg-white hover:text-dark-green flex items-center gap-2 w-1/4"
              >
                <font-awesome-icon :icon="['fas', 'chart-column']" class="text-3xl" />
                <p>Metricas</p>
              </RouterLink>
            </div>
          </div>
    ...
    >
      <section
        v-if="messageToggle"
        class="fixed w-full h-screen z-50 top-0 bg-rolex-green/60"
      ></section>
    </transition>
    ...
    <transition name="bounce">
      <div
        v-if="messageToggle"
        class="fixed w-full h-screen z-50 top-0 flex justify-center items-center"
      >
        <div
          class="border border-main-green bg-white rounded-xl shadow-sm p-3 w-1/2 relative"
        >
          <button
            @click="messageToggle = false"
            class="absolute top-2 right-4 text-2xl text-rolex-green"
          >
          ...
          <p class="">
            Número de teléfono: +{{ messages.activeMessage.phone }}
          </p>
          <div class="relative italic text-lg">
            "{{ messages.activeMessage.message }}"
          </div>
          <p class="text-right font-light">
            {{ getFormattedDate(messages.activeMessage.createdAt) }}
          </p>
        </div>
      </div>
    </transition>
