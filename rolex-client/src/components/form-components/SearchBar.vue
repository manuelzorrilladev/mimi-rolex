...
<template>
  ...
  >
    <form
      ...
      class="w-full flex items-center justify-center h-20 relative"
    >
      ...
      <input
        ...
        name="search"
        id=""
        class="w-11/12 h-1/2 rounded-full bg-[#909a90] px-10 placeholder:text-rolex-green/75"
      />
      <button>
        <font-awesome-icon
    ...
    <div id="results" class="w-full flex flex-col items-center h-fit">
      <h2 class="w-11/12">Resultados:</h2>
      <h2 v-if="input.length < 3" class="text-center w-full py-8">
        Comienza a buscar
      </h2>
      <ul
        v-else-if="input.length >= 3 && suggestions.length > 0"
        class="w-11/12 pl-4 py-8"
      >
        <li
      ...
      <h2 v-else class="text-center w-full py-8">No hay resultados</h2>
    </div>
  </div>
</template>
