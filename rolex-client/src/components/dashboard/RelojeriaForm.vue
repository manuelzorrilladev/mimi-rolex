    ...
    <DashboardCard class="w-11/12">
      ...
      <template #content>
        ...
        <section class="min-h-[600px]">
          ...
          >
            <font-awesome-icon
              :icon="['fas', errorContent[statusCheck].icon]"
              class="bg-white rounded-full text-9xl"
            />
            ...
            >
              ...
              <div class="flex w-full gap-4">
                ...
                <InputField
                  name="description"
                  type="text"
                  label="Pequeña descripción del producto"
                  class="w-3/5"
                />
              </div>
            ...
            <div v-if="imageCheck.length">
              ...
              >
                <template #item="{ element, index }">
                  <div class=" group cursor-move overflow-hidden border-2 border-rolex-green-2 rounded-lg relative">
                    <img
                      :src="element.url"
                      :alt="element.name"
                      class="w-full h-24 object-cover  shadow-md transition-colors group-hover:scale-115 duration-300"
                    />

                    <div
                      class="absolute inset-x-0 bottom-0 bg-rolex-green bg-opacity-60 text-white text-xs p-1 truncate"
                    >
                      {{ element.name }}
                    </div>

                    <button
                      @click="imageCheck.splice(index, 1)"
                      class="absolute top-0 right-0 bg-red-500 text-white rounded-bl-lg cursor-pointer w-5 h-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity"
                    >
              ...
              <!-- <div
                v-if="imageCheck.length"
                class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4"
              >
                <div
                  v-for="(image, index) in imageCheck"
                  :key="index"
              ...
              >
                <h3 class="block mb-2 text-md font-medium text-green-900">
